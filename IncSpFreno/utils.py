...
def incFreno(incDBPath, minsup, sc, k, freqRange, res):
    ...
    #trees = sc.textFile("hdfs:/tree_{}/".format(int(incDBPath[-5])-1))
    #freqRange.map(lambda v: concatFreno(transData[v][1],trees[v])).collect()
    #freqRange.saveAsTextFile("hdfs:/tree_{}/".format(int(incDBPath[-5])))
...
def distFreno(inFile, minsup, sc, k, freqRange):
    ...
    res = freqRange.map(lambda v: runFreno(transData[v][1],minsup)).collect()
    #freqRange.saveAsTextFile("hdfs:/tree_0/")
